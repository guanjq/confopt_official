...
from models.baseline_models.equivariant_attention.fibers import Fiber, fiber2head
from models.baseline_models.equivariant_attention.from_se3cnn import utils_steerable
from models.utils import GaussianSmearing
from utils.profiling import *  # load before other local modules
...
def get_basis_and_r(edge_d, max_degree):
    ...
    r_ij = utils_steerable.get_spherical_from_cartesian_torch(edge_d)
    # Spherical harmonic basis
    Y = utils_steerable.precompute_sh(r_ij, 2 * max_degree)  # [B, N, K, 2J+1]
    # Equivariant basis (dict['d_in><d_out>'])
    basis = get_basis(Y, max_degree)
    # Relative distances (scalar)
    r = torch.sqrt(torch.sum(edge_d ** 2, -1, keepdim=True))
    return basis, r
    ...
    def __init__(self, f_in, f_out, self_interaction: bool = False, edge_dim: int = 0):
        ...
        for (mi, di) in self.f_in.structure:
            for (mo, do) in self.f_out.structure:
                self.kernel_unary[f'({di},{do})'] = PairwiseConv(di, mi, do, mo, feat_dim=edge_dim + 1)
...
class RadialFunc(nn.Module):
    ...
    def __init__(self, num_freq, in_dim, out_dim, feat_dim, mid_dim=32):
        """NN parameterized radial profile function.
        ...
        self.net = nn.Sequential(nn.Linear(self.feat_dim, self.mid_dim),
                                 BN(self.mid_dim),
                                 nn.ReLU(),
    ...
    def __repr__(self):
        return f"RadialFunc(feat_dim={self.feat_dim}, in_dim={self.in_dim}, out_dim={self.out_dim})"
...
class PairwiseConv(nn.Module):
    ...
    def __init__(self, degree_in: int, nc_in: int, degree_out: int,
                 nc_out: int, feat_dim, mid_dim=32):
        ...
        self.rp = RadialFunc(self.num_freq, nc_in, nc_out, self.feat_dim, self.mid_dim)
...
class G1x1SE3(nn.Module):
    ...
    def forward(self, features, **kwargs):
        output = {}
        # print('transform keys: ', list(self.transform.keys()))
        for k, v in features.items():
            # print(f'k: {k} trans k: {self.transform[str(k)].shape} v: {v.shape}')
...
class GMABSE3(nn.Module):
    ...
    def forward(self, v, k: Dict = None, q: Dict = None, G=None, **kwargs):
        ...
        with G.local_scope():
            ...
            # print('f_value structure: ', self.f_value.structure)
            # for kk, vv in G.edata.items():
            #     print(f'k: {kk} v: {vv.shape}')
...
class GSE3Res(nn.Module):
    ...
    def __init__(self, f_in: Fiber, f_out: Fiber, edge_dim: int = 0, div: float = 4,
        ...
        # print('GMAB_v: ', sum(p.numel() for p in self.GMAB['v'].parameters() if p.requires_grad))
        self.GMAB['k'] = GConvSE3Partial(f_in, self.f_mid_in, edge_dim=edge_dim)
        # print('GMAB_k: ', sum(p.numel() for p in self.GMAB['k'].parameters() if p.requires_grad))
        self.GMAB['q'] = G1x1SE3(f_in, self.f_mid_in)
        # print('GMAB_q: ', sum(p.numel() for p in self.GMAB['q'].parameters() if p.requires_grad))

        # Attention
        self.GMAB['attn'] = GMABSE3(self.f_mid_out, self.f_mid_in, n_heads=n_heads)
        # print('GMAB_att: ', sum(p.numel() for p in self.GMAB['attn'].parameters() if p.requires_grad))

        # Skip connections
        self.project = G1x1SE3(self.f_mid_out, f_out, learnable=learnable_skip)
        # print('project: ', sum(p.numel() for p in self.project.parameters() if p.requires_grad))
    ...
    def forward(self, features, G, **kwargs):
        ...
        #     print(f'v: k: {ke} v: {va.shape}')
        t2 = time.time()
        # print('v emb time: ', t2 - t1)

        k = self.GMAB['k'](features, G=G, **kwargs)
        # for ke, va in k.items():
        #     print(f'k: k: {ke} v: {va.shape}')
        t3 = time.time()
        # print('k emb time: ', t3 - t2)

        q = self.GMAB['q'](features, G=G)
        # for ke, va in q.items():
        #     print(f'q: k: {ke} v: {va.shape}')
        t4 = time.time()
        # print('q emb time: ', t4 - t3)

        # Attention
        z = self.GMAB['attn'](v, k=k, q=q, G=G)
        t5 = time.time()
        # print('att time: ', t5 - t4)

        # Skip + residual
        z = self.project(z)
        z = self.add(z, features)
        t6 = time.time()
        # print('proj time: ', t6 - t5)
